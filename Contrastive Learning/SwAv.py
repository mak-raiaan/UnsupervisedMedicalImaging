...
class SwAV(nn.Module):
    def __init__(self, backbone, output_dim=128, num_prototypes=3):
        super().__init__()
    ...
    def forward(self, images):
        ...
        assignments = torch.softmax(similarities, dim=1)

        loss = 0
        for i in range(batch_size):
            prototype_index = i % self.num_prototypes
            loss -= torch.log(assignments[i, prototype_index])
...
# Train the model
model = SwAV(Backbone(), num_prototypes=3)
